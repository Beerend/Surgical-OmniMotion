...
class NVPSimplified(nn.Module):
    def __init__(
        ...
        for i in self.layer_idx:
            ...
            proj = get_projection_layer(proj_dims=proj_dims, type=proj_type, pe_freq=pe_freq, device=device)
...
class FixedPositionalEncoding(ProjectionLayer):
    def __init__(self, input_dims, frequency, proj_dims, device='cuda'):
        super().__init__(input_dims, proj_dims)
        ll = frequency
        self.sigma = np.pi * torch.pow(2, torch.linspace(0, ll - 1, ll, device=device)).view(1, -1)
        self.proj = nn.Sequential(
...
def get_projection_layer(**kwargs):
    ...
    if type == "identity":
        ...
        return FixedPositionalEncoding(2, kwargs.get("pe_freq", 4), kwargs.get("proj_dims", 128), device=kwargs["device"])
