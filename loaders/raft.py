...
class RAFTExhaustiveDataset(Dataset):
    def __init__(self, args, max_interval=None):
        ...
        self.tool_mask_dir = os.path.join(self.seq_dir, 'mask', args.tool_mask_dir) if args.tool_mask_dir is not None else None
    ...
    def __getitem__(self, idx):
        ...
        swap_images = np.random.choice([0, 1])
        if swap_images:
            id1, id2, pts1, pts2, gt_rgb1, gt_rgb2 = id2, id1, pts2, pts1, gt_rgb2, gt_rgb1
            weights[covisible_mask == 0.] = 0
            
        # read tool mask as loss weighting
        if self.tool_mask_dir is not None:
            img_name = img_name2 if swap_images else img_name1
            tool_mask = imageio.imread(os.path.join(self.tool_mask_dir, img_name))[..., -1] / 255.
            tool_weights = np.where(tool_mask > 0., self.args.tool_factor, 1.)
            tool_weights = torch.from_numpy(tool_weights[mask][select_ids][ :, np.newaxis]).float()
        else:
            tool_weights = torch.ones_like(weights)
        ...
        data = {'ids1': id1,
                ...
                'tool_weights': tool_weights  # [n_pts, 1]
                }
        return data