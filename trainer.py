...
import util
from criterion import masked_mse_loss, masked_l1_loss, compute_depth_range_loss, lossfun_distortion, weighted_mse_loss
...
class BaseTrainer():
    ...
    def compute_all_losses(self,
        ...
        tool_weights = batch['tool_weights'].to(self.device)
        # tool_weights = tool_weights.to(self.device) if tool_weights is not None else None
        ...
        if mask.sum() > 0:
            ...
            # Tool-weights loss functions
            # rgb_tool_weights = tool_weights[rgb_mask] if tool_weights is not None else None
            loss_rgb = weighted_mse_loss(pred_rgb1[rgb_mask], gt_rgb1[rgb_mask], tool_weights[rgb_mask])
            loss_rgb_grad = self.gradient_loss(pred_rgb1[rgb_mask], gt_rgb1[rgb_mask], tool_weights[rgb_mask])
            # flow_tool_weights = weights[mask] * tool_weights[mask] if tool_weights is not None else weights[mask]
            optical_flow_loss = masked_l1_loss(px2s_proj[mask], px2s[mask], weights[mask] * tool_weights[mask], normalize=False)
            optical_flow_grad_loss = self.gradient_loss(px2s_proj[mask], px2s[mask], weights[mask] * tool_weights[mask])
        else:
        ...
        with torch.no_grad():
            ...
            for id in range(0, self.num_imgs):
                if vis_occlusion:
                    if id == query_id:
                        kp_i = query_kpt
                        kp_depth = torch.zeros_like(query_kpt[..., :1]) if return_depth else None
                        occlusion_score = torch.zeros_like(query_kpt[..., :1])
                    else:
                        ret = self.get_correspondences_and_occlusion_masks_for_pixels([query_id], query_kpt[None], [id],
                                                                                    return_depth=return_depth, use_max_loc=use_max_loc)
                        if return_depth:
                            kp_i, occlusion_score, kp_depth = ret
                            kp_depth = kp_depth[0]
                        else:
                            kp_i, occlusion_score = ret
                        kp_i = kp_i[0]
                    ...
                    if id == query_id:
                        ...
                        ret = self.get_correspondences_for_pixels([query_id], query_kpt[None], [id],
                                                                  return_depth=return_depth, use_max_loc=use_max_loc)
                        if return_depth:
                            kp_i, kp_depth = ret
                            kp_depth = kp_depth[0]
                ...
                kp_i = torch.cat([kp_i, kp_depth], dim=-1) if return_depth else kp_i
                frames.append(out)
                kpts.append(kp_i)
    ...
    def eval_video_correspondences(self, query_id, pts=None, num_pts=200, seed=1234, use_mask=False,
                                   ...
                                   interval=10, radius=2, return_kpts=False, return_depth=False):
        with torch.no_grad():
            if mask is not None:
